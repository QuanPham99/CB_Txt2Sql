...
- **curl**.
- macOS hoặc Linux, hoặc **WSL2** trên Windows — các lệnh cài đặt trong `postCreate.sh` dựa trên Bash/curl và giả định một shell kiểu Unix. Windows thuần (cmd/PowerShell) không được hỗ trợ ở đây. Nếu máy chưa có gì cài sẵn (Git, Node, WSL2, ...), xem `../participants/LOCAL_SETUP_GUIDE.md` (Phần A) — có hướng dẫn từng bước dành cho một trợ lý kỹ thuật làm cùng người tham gia không rành kỹ thuật.
...
> **Cài đặt từ máy hoàn toàn mới đã chuyển sang tài liệu dành cho người tham
> gia:** hướng dẫn từng bước cho Windows/macOS trên một máy chưa từng cài gì
> (dành cho trợ lý kỹ thuật hỗ trợ trực tiếp một người tham gia không rành kỹ
> thuật) giờ nằm ở `../participants/LOCAL_SETUP_GUIDE.md`, Phần A. Nếu máy của
> bạn đã có Git/Node/git-lfs, bỏ qua và tiếp tục ngay bên dưới.
...
OAuth cục bộ trên máy của chính bạn thường là con đường đăng nhập *mượt nhất* — không có các trục trặc redirect qua forwarded-port như Codespaces. Nếu một trong hai bị lặp vòng đăng nhập, xem hướng dẫn OAuth chung trong `FACILITATOR_TROUBLESHOOTING.md` (SSO tổ chức chặn redirect là nguyên nhân phổ biến nhất, không phụ thuộc vào việc dùng Codespaces hay cục bộ).
...
> **Dữ liệu tùy chỉnh cục bộ (CSV của riêng bạn) đã chuyển sang tài liệu dành
> cho người tham gia:** `../participants/LOCAL_SETUP_GUIDE.md`, Phần C.
...
| `WORKSHOP_SETUP_PLAN.md` | Toàn bộ bối cảnh/lý do của ban tổ chức cho tất cả những điều trên |
| `DAY_OF_CHECKLIST.md` | Checklist trước workshop và hậu cần ngày diễn ra |
| `FACILITATOR_TROUBLESHOOTING.md` | Xử lý sự cố trực tiếp trong buổi (chỉ dành cho facilitator) |
| `../participants/LOCAL_SETUP_GUIDE.md` | Cài đặt từ máy mới + dữ liệu tùy chỉnh cục bộ, phiên bản dành cho người tham gia |
