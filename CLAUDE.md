...
There is no lint/build/test command — changes are validated by actually asking the `sql-helper` skill a question (e.g. via `claude` or `codex` in the repo root) and confirming it picks the correct schema doc under `schemas/` for the question asked, shows its SQL, and returns correct numbers. `organizers/WORKSHOP_SETUP_GUIDE_LOCAL.md` and `organizers/WORKSHOP_SETUP_GUIDE_CODESPACES.md` document this verification flow step by step.

## Architecture

For a full system-level walkthrough (build-time vs. run-time pipelines, query lifecycle, component map, design-tradeoff rationale) see `participants/ARCHITECTURE.md` — read it before making any structural change to the devcontainer, skill mirroring, or data pipeline. The summary below covers what's needed for day-to-day edits.
...
**Second dataset pipeline (tech-salary)**: `tech_salary_dataset/*.csv` (git-ignored, organizer-only input) → `organizers/build_tech_salary_db.sh` (organizer-only, fail-fast on any malformed CSV, always rebuilds from scratch) → `data/tech_salary.duckdb` (LFS-tracked, already covered by the existing `data/*.duckdb` `.gitattributes` pattern) → hand-authored `schemas/TECH_SALARY_DATASET_SCHEMA.md`. Fully independent of the banking pipeline — no shared tables, no cross-dataset joins expected. `postCreate.sh` and `participants/local_setup/setup.sh` both smoke-test it the same way they smoke-test `workshop.duckdb` (generically, via `information_schema.tables`, since its table names aren't hardcoded anywhere). `sql-helper` itself already routes between the two datasets (see above); the second exercise (`WORKSHOP_EXERCISES.md`, last section) has participants add that same routing to their *own* skill (built in the first exercise, still single-dataset at that point) in place, rather than build a second skill — there is deliberately no second worked-example skill folder.

**Local custom-data pipeline**: `my-data/*.csv` (git-ignored, participant-owned) → `participants/local_setup/load_custom_data.sh` (participant-facing, CSV-only, always rebuilds `data/custom.duckdb` from scratch, per-file error isolation) → `data/custom.duckdb` (git-ignored, never committed — distinct from the two datasets above, which are LFS-tracked). `templates/custom-data-skill-template/SKILL.md` is the matching blank scaffold, deliberately without a schema doc to reference — participants explore their own schema via `.tables`/`DESCRIBE` and write it into their own SKILL.md by hand. Excel/.xlsx is intentionally out of scope here (handled by a separate "Claude for Excel" product, introduced outside this repo). Local install for this track is a manual, human-walked-through guide — participant-facing steps live in `participants/LOCAL_SETUP_GUIDE.md` (Phần A: install from a fresh machine, Phần C: the custom-data track itself), paired with a facilitator runbook in `organizers/FACILITATOR_TROUBLESHOOTING.md` — rather than an automated installer script; there is no `setup.ps1` or auto-installing prerequisite logic anywhere in this repo.
...
**`participants/` and `organizers/` are split top-level folders by audience**, not by file type:
- `participants/` — everything a participant may need to read: `ARCHITECTURE.md` (system-level deep dive referenced above), `exercises_answer_key_bank_schema.ipynb` (worked answers to `WORKSHOP_EXERCISES.md`'s bank-dataset exercise, for self-checking) and `exercises_answer_key_salary_schema.ipynb` (the same, for the salary-dataset exercise — same Cơ bản/Trung cấp/Nâng cao structure, one dedicated notebook per schema), `LOCAL_SETUP_GUIDE.md` (the participant-facing half of the local/custom-data track), and `local_setup/` (holds `setup.sh` and `load_custom_data.sh` — see Commands above; both resolve `data/`/`my-data/` relative to the repo root via their own script path, so they behave identically to before regardless of this nesting). `participants/README.md` indexes all of it.
- `organizers/` — organizer-facing setup plan, dry-run guides, and logistics: `WORKSHOP_SETUP_PLAN.md` is the rationale doc everything else links back to; `WORKSHOP_SETUP_GUIDE_LOCAL.md` and `WORKSHOP_SETUP_GUIDE_CODESPACES.md` are dry-run verification checklists for the two ways to run the workshop; `Iteration_0_LocalTesting.md` is the Docker/devcontainer-parity local test; `DAY_OF_CHECKLIST.md` and `FACILITATOR_TROUBLESHOOTING.md` are used live during/around the session (deliberately not linked from the participant-facing README); `build_tech_salary_db.sh` is the organizer-only dataset build script. `organizers/README.md` indexes all of it.
- `README.md`, `WORKSHOP_EXERCISES.md` stay in the repo root because `SKILL.md`, `postAttach.sh`, and the setup guides all reference them by that bare path.
- `schemas/` holds the hand-authored data dictionaries (`BANK_DATASET_SCHEMA.md`, `TECH_SALARY_DATASET_SCHEMA.md`) — kept in their own top-level folder, not under `participants/`, because they're read by `sql-helper` itself at answer time (not just by humans) and are meant to grow by simply adding a new file, not by editing the audience-organized tree.
...
These docs cross-reference each other by relative path (e.g. `organizers/*.md` guides link sideways to `participants/*.md` and back). When moving or renaming any `.md` file, grep the whole repo for its filename and fix every reference — nothing enforces these links automatically.
