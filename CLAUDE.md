...
- `participants/` — everything a participant may need to read: `ARCHITECTURE.md` (system-level deep dive referenced above), `exercises_answer_key_bank_schema.ipynb` (worked answers to `exercises.md`'s bank-dataset questions, for self-checking) and `exercises_answer_key_salary_schema.ipynb` (the same, for the optional Tech Salary dataset — same Cơ bản/Trung cấp/Nâng cao structure, one dedicated notebook per schema), and `LOCAL_SETUP_GUIDE.md` (the participant-facing half of the local/custom-data track). `participants/README.md` indexes all of it.
